...
hr_region <- read_rds("data/hr/hr_region.rds") # Load in Hampton Roads geojson as rds.
...
hr_pop_total <- read_rds("data/hr/hr_pop.rds") |> # Load in population data for Hampton Roads.
...
ggplot(hr_var_dom,
  ...
  geom_point() +
  theme_hfv() +
  geom_text(data = subset(hr_var_dom, name %in% c("Williamsburg",  "VB-Norfolk-NN MSA", "Surry County")),aes(label = med_dom, hjust = ifelse(period == "2023 Q2", 1.50, -0.5))) +
  coord_flip() +
