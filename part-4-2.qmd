...
rva_tracts <- st_read("data/planrva/geo/rva_race_tracts_22.gpkg") |> 
  st_transform(4326) |> 
  filter(percent >= 0.7)
...
leaflet() |> 
  ...
  addPolygons(data = rva_tracts,
              ...
              weight = 1,
              fillOpacity = 0.2,
              popup = rva_tracts$name_long) |> 
  addPolygons(data = region,
              color = "#66788d", # Set polygon fill color.
              weight = 1, # Set polygon outline weight.
              fillOpacity = 0, # Set polygon fill color transparency.
              popup = region$Jurisdiction) |> # Set pop-up to jurisdiction name.
  addLegend(data = rva_tracts,
            labels = "70% or more white, non-hispanic census tract",
            color = "#40C0C0",
            group = "70% or more white") |> 
  addLegend(data = by4,
            labels = "Multifamily (4+) By-Right",
            color = "#B1005F",
            group = "Multifamily (4+) zoning") 
