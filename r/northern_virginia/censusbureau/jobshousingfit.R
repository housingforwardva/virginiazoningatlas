...
# Northern VA localities fips codes.
...
b25061_raw <- get_acs(
  ...
  subset(fips %in% nova_fips) # Filter for Hampton Roads
...
nova_od_blocks <- read_rds("data/va_od_block.rds") |>
...
write_rds(nova_od_blocks, "data/nova/nova_od_blk_grps.rds")

nova_od <- read_rds("data/nova/nova_od_blk_grps.rds")
...
nova_blk_grps <- block_groups("VA", cb = TRUE) |>
  mutate(geoid = GEOID) |> 
  right_join(nova_od, by = "geoid") |> 
...
write_rds(nova_ratio, "data/nova/nova_ratio.rds")
write_rds(nova_blk_grps, "data/nova/nova_blk_grps.rds")
...
ratio_bins <- c(0, 0.5, 1, 1.5, 2, 10, 50, 100, 300, 500, 700)

pal <- colorBin(palette = "Reds", bins = ratio_bins, domain = (nova_blk_grps$ratio), na.color = "#011E41")

nova_jhfit <- leaflet(nova_blk_grps,) |> 
  addPolygons(
              fillColor = ~pal(ratio),
              fillOpacity = 0.8,
              color = "white",
...
write_rds(nova_jhfit, "data/nova/nova_jhfit_map.rds")

nova_jhfit


mapview(nova_blk_grps, zcol = "ratio")
