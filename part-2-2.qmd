...
ggplot(hr_hhsize,
  ...
  coord_flip() +
  scale_fill_hfv() +
  labs(title = "Change in household size by tenure in Hampton Roads",
...
hr_emp <- read_rds("data/hr/hr_oews.rds") |> 
  select(year, emp = annual_avg_emplvl, name_long) |> 
  group_by(year, name_long) |> 
  summarise(emp = sum(emp)) |> 
  mutate(year = as.character(year))

hr_hu <- read_rds("data/hr/hr_hu.rds") |> 
  select(year, value, name_long) |> 
  mutate(value = as.numeric(value)) |> 
  group_by(year, name_long) |> 
  summarise(value = sum(value))

hr_job_housing <- hr_emp |> 
  inner_join(hr_hu, by = c("name_long", "year")) |> 
  mutate(ratio = emp/value) |> 
  filter(year == "2022" | year == "2016")

ggplot(hr_job_housing,
