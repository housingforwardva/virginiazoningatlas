...
hr_zoning_summary <- read_csv("data/hr/csv/hr_zoning_summary.csv")
...
hr_type <- read_rds("data/hr/rds/hr_vza_nogeo.rds") |> 
  select(jurisdiction, overlay, type, acres) |> 
  filter(overlay == 0)
...
Roughly 91 percent of developable land is [Primarily Residential]{.primary-res} . This includes prime agricultural land in the southern parts of Hampton Roads, where the preservation of open space and natural resources is a vital part of the region. Only three percent of the region's developable land is classified as [Mixed with Residential]{.mixed-res}. Locally, **Williamsburg (23%), York County (18%), and Newport News (12%)** have the **largest proportions of** [Mixed with Residential]{.mixed-res} **zoning types**, while Surry County (0%), Southampton County (0%), and Isle of Wight County (less than 1%) have the least.
...
hr_sfd_map <- read_rds("data/hr/rds/hr_sfd_only_map.rds")
...
hr_sfd <- read_rds("data/hr/rds/hr_vza_nogeo.rds") |> # Read in the data.
  ...
  ungroup() |> 
  mutate(total_hr = sum(acres)) |> # Calculate what percentage of total developable area is SFD.
  filter(sfd == "t") |> 
  add_row(jurisdiction = "York", sfd = "t", percent = 0, total_hr = 1425279)

hr_sfd_summary <- hr_sfd |> 
  group_by(total_hr) |> 
  summarise(total_sfd = sum(acres)) |> 
  mutate(pct = total_sfd/total_hr)
...
hr_lots <- read_csv("data/hr/csv/hr_sfd_minlot.csv") |> 
...
hr_byright_tbl <- read_rds("data/hr/rds/hr_vza_nogeo.rds") |> 
...
hr_byright <- read_rds("data/hr/rds/hr_vza_nogeo.rds") |> 
...
hr_byright_summary <- read_rds("data/hr/rds/hr_vza_nogeo.rds") |> 
  select(jurisdiction, abbrvname, overlay, 6:9, acres) |> # Select data fields.
  filter(overlay == 0) |> # Filter out overlay districts.
  mutate(total_area = sum(acres)) |> # Calculate the total area of zoned land.
  filter(family2_treatment == "allowed" & family3_treatment == "allowed" & family4_treatment == "allowed") |> 
...
hr_byr2 <- read_rds("data/hr/rds/hr_byr_map.rds")
...
hr_parking <- read_csv("data/hr/csv/hr_vza_parking.csv")
...
hr_adu <- read_rds("data/hr/rds/hr_vza_nogeo.rds") |> 
  ...
  summarise(acres = sum(acres)) |> 
  mutate(pct = acres/total_area) |> 
  ungroup() |> 
  add_row(jurisdiction = "Surry (town)", total_area = 0, acres = 0, pct = 0) |> 
  add_row(jurisdiction = "Williamsburg", total_area = 0, acres = 0, pct = 0)
...
hr_adu <- read_rds("data/hr/rds/hr_vza_nogeo.rds") |> 
  ...
  add_row(jurisdiction = "Surry (town)", total_area = 0, acres = 0, pct = 0) |> 
  add_row(jurisdiction = "Newport News", total_area = 0, acres = 0, pct = 0) |> 
  add_row(jurisdiction = "Chesapeake", total_area = 0, acres = 0, pct = 0) |> 
  add_row(jurisdiction = "Windsor", total_area = 0, acres = 0, pct = 0) |> 
  add_row(jurisdiction = "Smithfield", total_area = 0, acres = 0, pct = 0) |> 
  add_row(jurisdiction = "Williamsburg", total_area = 0, acres = 0, pct = 0)
...
hr_adu_summary <- read_csv("data/hr/csv/hr_adu_summary.csv") |> 
...
hr_adu_map <- read_rds("data/hr/rds/hr_adu_map.rds")
...
hr_transit_map <- read_rds("data/hr/rds/hr_transit_map.rds")
...
housing_needed <- read_rds("data/hr/rds/hr_blk_grps.rds") |> 
  filter(ratio > 2 | is.na(ratio))

pal <- colorFactor(palette =c("#8B85CA", "#40C0C0"),levels = c("R", "M"))

hr_byr4 <- read_rds("data/hr/rds/hr_byr4_map.rds") 
