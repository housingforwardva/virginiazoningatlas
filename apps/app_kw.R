...
ui <- page_fluid(
  ...
  fluidRow( 
    ...
    absolutePanel(
      ...
      shiny::textInput("geocode", label = NULL, placeholder = "Enter address"),
      shiny::actionButton("zoom", label = "Zoom to address"),
...
server <- function(input, output, session) {
  ...
  geo_outputs <- eventReactive(input$zoom, {
    loc <- mapboxapi::mb_geocode(input$geocode, output = "sf") |> 
      dplyr::mutate(icon = "marker",
                    address = input$geocode)
    
    loc_buffer <- sf::st_buffer(sf::st_transform(loc, 3968), 5000) |> 
